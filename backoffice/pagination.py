...
class CustomDatatablesPagination(DatatablesPageNumberPagination):
    ...
    def get_paginated_response(self, data):
        ...
        order_column_index = request.query_params.get("order[1][column]", 1)  # Index de colonne triée
...
class DatatablesPagination(PageNumberPagination):
    page_size_query_param = 'length'  # Permet de spécifier la taille de la page avec ?length=5
    page_query_param = 'page'  # Utilise 'page' pour définir la page actuelle
    start_query_param = 'start'  # Prend en charge 'start' comme dans DataTables
    
    def paginate_queryset(self, queryset, request, view=None):
        start = request.query_params.get(self.start_query_param)
        length = request.query_params.get(self.page_size_query_param, self.page_size)
        
        if start is not None:
            start = int(start)
            length = int(length)
            self.page = (start // length) + 1  # Convertir start en numéro de page
        
        return super().paginate_queryset(queryset, request, view)

    def get_paginated_response(self, data):
        return Response({
            'draw': self.request.query_params.get('draw', 1),  # Compatible avec DataTables
